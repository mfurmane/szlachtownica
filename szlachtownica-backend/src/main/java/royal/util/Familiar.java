...
//		printFamilies();
	...
	public static void repairBloodlines(Family family) {
		double bestBloodline = 0;
		for (int i = 0; i < family.aliveMembers.size(); i++) {
			Person member = family.aliveMembers.get(i);
			if (bestBloodline < member.lineImportance)
				bestBloodline = member.lineImportance;
		}
		for (int i = 0; i < family.aliveMembers.size(); i++) {
			Person member = family.aliveMembers.get(i);
			if (member.status != Status.ACCEPTED_BASTARD && member.status != Status.UNKNOWN_FATHER) {
				member.lineImportance *= (1 / bestBloodline);
				member.importance = member.family.strenght * member.lineImportance / 10;
			}
		}
		...
		if (!garvonI.isMarried() && Timer.currentCalendar.after(newWife)) {
			Person newWifeObj = new Person(Namer.getName(true, Racist.human), humanPlebs, null, null,
					...
					0.1 + 0.3 * rand.nextDouble(), 1, 1, 17, 19, 1, 1, 0, Stater.jealous(null), Stater.impulsive(null),
					Stater.proud(null), Stater.amorous(null));
			newWifeObj.plannedKids = 0;
			Marriager.marriage(garvonI, newWifeObj);
		}
		if (Timer.currentCalendar.after(killDate)) {
			if (family == tagar) {
				for (int i = 0; i < family.aliveMembers.size(); i++) {
					Person person = family.aliveMembers.get(i);
					if (rand.nextDouble() < 0.1 && !person.name.equals("Eol II")) {
						person.died = (Calendar) Timer.currentCalendar.clone();
